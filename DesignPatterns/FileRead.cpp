...
void FileRead::fileOpen(std::ifstream& file, const std::string& fileName) {
    file.open(fileName);
    if (!file.is_open()) {
...
void FileRead::fileClose(std::ifstream& file) {
    if (file.is_open()) {
        file.close();
    }
}

void FileRead::fileRead(std::ifstream& file, std::map<std::string, 
    std::string>& nodes, std::map<std::string, 
